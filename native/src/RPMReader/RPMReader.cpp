...
void _rpm_in_handler() { rpm_in_cnt++; }
void _rpm_out_handler() { rpm_out_cnt++; }
...
rpm_values_t _recent_values;
rpm_values_t rpm_read()
{
    auto millis_diff = millis() - _last_millis;
    if (millis_diff > 2000)
    {
        _recent_values.in = 1000 * 60 * rpm_in_cnt / millis_diff / 2;
        _recent_values.out = 1000 * 60 * rpm_out_cnt / millis_diff / 2;
